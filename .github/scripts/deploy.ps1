<#  
    Despliega un build Unity headless en C:\GameServer  
...
# 1b) Detener servicio si está corriendo (evita locks de archivos)
if (Get-Service -Name $serviceName -ErrorAction SilentlyContinue) {
    Write-Host "Stopping $serviceName to release file locks..."
    Stop-Service -Name $serviceName -Force -ErrorAction SilentlyContinue
    try { Wait-Service -Name $serviceName -Status Stopped -Timeout 20 } catch {}
}
...
# 6) Limpiar logs NSSM vacíos
Get-ChildItem "$targetDir\service-*.log" -ErrorAction SilentlyContinue | Remove-Item -Force
